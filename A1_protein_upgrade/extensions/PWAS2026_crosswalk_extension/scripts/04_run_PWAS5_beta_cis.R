...
candidates <- fread(file.path(extension_root, "data_processed", "PWAS5_pqtl_candidates.tsv"),
                    na.strings = c("NA", ""))
candidates <- candidates[cis_to_encoding_gene == TRUE & rsid_mapping_status == "mapped"]
members <- fread(file.path(extension_root, "config", "PWAS5_frozen_members.tsv"))
planned_assays <- unique(members[, .(gene_symbol, assay_target_ID = UKB_PPP_OID, UniProt_ID)])
planned_family_size <- nrow(planned_assays)
planned_family_total <- planned_family_size * 4L
output_file <- file.path(extension_root, "tables", "PWAS5_beta_cis.tsv")
iv_file <- file.path(extension_root, "data_processed", "PWAS5_instruments_cis.tsv")
harm_file <- file.path(extension_root, "tables", "PWAS5_harmonized_instruments_cis.tsv")
log_dir <- file.path(extension_root, "logs", "beta_cis")
...
for (outcome_name in names(outcome_specs)) {
  ...
  for (gene in names(exposures)) {
    ...
    estimate[, `:=`(
      ...
      beta_source = "PWAS2026_crosswalk_extension_same_assay", availability_scope = "reestimated_subset",
      analysis_set = "PWAS5_cis_only_sensitivity", planned_family_total = planned_family_total,
...
planned_grid <- CJ(gene_symbol = planned_assays$gene_symbol, outcome = names(outcome_specs), unique = TRUE)
observed_grid <- unique(result[, .(gene_symbol, outcome)])
missing_grid <- planned_grid[!observed_grid, on = .(gene_symbol, outcome)]
if (nrow(missing_grid)) {
  missing_grid[, `:=`(
    method = "not_estimable", method_role = "primary", nsnp = 0L,
    beta = NA_real_, SE = NA_real_, P_value = NA_real_, CI_lower = NA_real_, CI_upper = NA_real_,
    OR = NA_real_, OR_CI_lower = NA_real_, OR_CI_upper = NA_real_, Cochran_Q = NA_real_,
    Q_df = NA_real_, Q_P = NA_real_, Egger_intercept = NA_real_, Egger_intercept_SE = NA_real_,
    Egger_intercept_P = NA_real_, Steiger_direction = NA, Steiger_P = NA_real_,
    Steiger_method = "not_estimable", mean_F = NA_real_, min_F = NA_real_,
    planned_family_size = planned_family_size, observed_primary_tests_in_family = NA_integer_,
    P_FDR_observed_reestimated = NA_real_, P_Bonferroni_planned_family = NA_real_,
    corrected_significance_FDR_observed = NA, corrected_significance_Bonferroni_planned = NA,
    beta_status = "not_estimable", exclusion_reason = "No cis pQTL remained after selection, clumping or harmonization.",
    beta_source = "PWAS2026_crosswalk_extension_same_assay", availability_scope = "planned_frozen_set",
    analysis_set = "PWAS5_cis_only_sensitivity", planned_family_total = planned_family_total,
    P_FDR_all_estimable = NA_real_, P_Bonferroni_planned_20 = NA_real_,
    corrected_significance_FDR_all_estimable = NA,
    corrected_significance_Bonferroni_planned_20 = NA
  )]
  result <- rbindlist(list(result, missing_grid), fill = TRUE)
}
...
all_primary <- which(result$method_role == "primary" & result$beta_status == "reestimated_cis_only")
if (length(all_primary)) {
  result[all_primary, P_FDR_all_estimable := p.adjust(P_value, "BH")]
  result[all_primary, P_Bonferroni_planned_20 := pmin(1, P_value * planned_family_total)]
  result[all_primary, corrected_significance_FDR_all_estimable := P_FDR_all_estimable < 0.05]
  result[all_primary, corrected_significance_Bonferroni_planned_20 := P_Bonferroni_planned_20 < 0.05]
}
