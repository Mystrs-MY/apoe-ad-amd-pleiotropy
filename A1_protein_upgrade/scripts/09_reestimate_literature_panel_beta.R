...
candidate_file <- file.path(upgrade_root, "data_processed", "literature_panel_pqtl_candidates.tsv")
provenance_file <- file.path(upgrade_root, "tables", "Table_Literature_Prioritized_Protein_Provenance.tsv")
mapping_file <- file.path(upgrade_root, "tables", "APOE_linkable_target_assay_mapping.tsv")
output_file <- file.path(upgrade_root, "tables", "literature_panel_beta_results.tsv")
iv_output <- file.path(upgrade_root, "data_processed", "literature_panel_reestimated_IVs.tsv")
harm_output <- file.path(upgrade_root, "data_processed", "literature_panel_harmonized_data.tsv")
log_dir <- file.path(upgrade_root, "logs", "beta_reestimation")
...
mapping <- fread(mapping_file, na.strings = c("NA", ""))
planned_assays <- unique(mapping[tolower(eligible_for_primary) == "true",
                                 .(gene_symbol = standardized_gene_symbol, assay_target_ID = UKB_PPP_OID)])
...
ordered_columns <- c(
  ...
  "corrected_significance_Bonferroni_planned", "beta_status", "exclusion_reason", "beta_source"
)
